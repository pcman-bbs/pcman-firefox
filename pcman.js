...
*/
    ...
    paste: function(selection) {
        ...
        text = text.replace(/\r\n/g, '\r');
        text = text.replace(/\n/g, '\r');
        text = text.replace(/\r/g, UnEscapeStr(prefs.EnterKey));
        if(text.indexOf('\x1b') < 0 && prefs.LineWrap > 0)
            text = wrapText(text, prefs.LineWrap, UnEscapeStr(prefs.EnterKey));
        //FIXME: stop user from pasting DBCS words with 2-color
        text = text.replace(/\x1b/g, UnEscapeStr(prefs.EscapeString));
...
*/
    ...
    onMenuPopupShowing : function () {
      var copy = document.getElementById("popup-copy");
      var coloredCopy = document.getElementById("popup-coloredCopy");
      var searchMenu = document.getElementById("popup-search");
      var hasSelection = pcman.view.selection.hasSelection();
